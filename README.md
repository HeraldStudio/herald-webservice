...
如果某个上游一直不可用，请求会拥塞在 WebService 上，导致网络变得极其缓慢。因此引入了熔断器（circuit，完整的叫法为 Circuit Breaker）中间件，当某个路由在一分钟的时间窗内连续三次出现如下情况之一：
1. 抛出 408 或 503 异常（即 Axios 内部的未捕获错误和超时）
2. 执行超过十秒后抛出任意异常

则会对该路由执行强制性的拒绝服务措施（称作「熔断」），并直接抛出 503，不再执行下游中间件。当最先发生的一次失败移出了这个 1 分钟的时间窗，导致这个时间窗内不再有足够三次实际出错时，熔断器将对该路由允许一次服务（称作「探测」），如果依然失败，则保持熔断状态；如果发现成功，则熔断解除。
...
```SQL
  CREATE TABLE "TOMMY"."H_AUTH" 
   (	"TOKEN_HASH" VARCHAR2(300 BYTE) NOT NULL ENABLE, 
	"CARDNUM" VARCHAR2(20 BYTE) NOT NULL ENABLE, 
	"REAL_NAME" VARCHAR2(60 BYTE) NOT NULL ENABLE, 
	"CREATED_TIME" TIMESTAMP (6) NOT NULL ENABLE, 
	"LAST_INVOKED_TIME" TIMESTAMP (6) NOT NULL ENABLE, 
	"PLATFORM" VARCHAR2(60 BYTE) NOT NULL ENABLE, 
	"SCHOOLNUM" VARCHAR2(20 BYTE), 
	 CONSTRAINT "H_AUTH_PK" PRIMARY KEY ("TOKEN_HASH")
  ...
  TABLESPACE "USERS" ;
   ...
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."TOKEN_HASH" IS 'api-token的sha(256)哈希';
 
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."CARDNUM" IS '一卡通号';
 
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."REAL_NAME" IS '姓名';
 
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."CREATED_TIME" IS '认证时间';
 
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."LAST_INVOKED_TIME" IS '最近调用时间';
 
   COMMENT ON COLUMN "TOMMY"."H_AUTH"."PLATFORM" IS '登录平台信息';
 
   COMMENT ON TABLE "TOMMY"."H_AUTH"  IS '认证表';
```

### H_ADMIN 管理员表

```SQL
  CREATE TABLE "TOMMY"."H_ADMIN" 
   (	"ID" VARCHAR2(40 BYTE) DEFAULT sys_guid() NOT NULL ENABLE, 
	"CARDNUM" VARCHAR2(20 BYTE) NOT NULL ENABLE, 
	"PHONENUM" VARCHAR2(20 BYTE) NOT NULL ENABLE, 
	"ACCESS_LEVEL" NUMBER(*,0) NOT NULL ENABLE, 
	"LAST_INVOKED_TIME" TIMESTAMP (6), 
	"REAL_NAME" VARCHAR2(60 BYTE), 
	 CONSTRAINT "H_ADMIN_PK" PRIMARY KEY ("ID")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "USERS"  ENABLE
...
### H_ADMIN_PERMISSION 管理员权限表

```SQL
  CREATE TABLE "TOMMY"."H_ADMIN_PERMISSION" 
   (	"ID" VARCHAR2(40 BYTE) DEFAULT sys_guid() NOT NULL ENABLE, 
	"CARDNUM" VARCHAR2(20 BYTE) NOT NULL ENABLE, 
	"PERMISSION" VARCHAR2(40 BYTE) NOT NULL ENABLE, 
	 CONSTRAINT "H_ADMIN_PERMISSION_PK" PRIMARY KEY ("ID")
  ...
  TABLESPACE "USERS" ;
   ...
   COMMENT ON COLUMN "TOMMY"."H_ADMIN_PERMISSION"."CARDNUM" IS '一卡通号';
 
   COMMENT ON COLUMN "TOMMY"."H_ADMIN_PERMISSION"."PERMISSION" IS '赋予的权限';
```
