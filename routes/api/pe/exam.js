...
exports.route = {
  async get() {
    let resources = await this.publicCache('1mo', async () => {
      ...
      return $('a[data-type="pdf"]').toArray().map(k => $(k)).map(k => ({
    ...
    return await this.userCache('1mo', async () => {
      ...
      let res = await this.get(`http://xk.urp.seu.edu.cn/jw_service/service/stuCurriculum.action?queryStudentId=${ schoolnum }`)
