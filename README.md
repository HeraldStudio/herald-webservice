...
如果某个上游一直不可用，请求会拥塞在 WebService 上，导致网络变得极其缓慢。因此，通过 axios API 发出的请求将会记忆状态，若对某个 url（不含 query）的请求在一分钟之内出现过五次超时，将不再接受对这个 URL 的请求，而是直接抛出 503。这样保证了对于超时的上游，任意一分钟时间窗内实际发出的请求不会超过五次。
