...
`lazy` 策略表示，只要缓存存在且可以解析（无论是否过期），每个请求都立即返回上次缓存的值；然后如果缓存过期，在后台回源调用下游中间件。

为了方便理解，懒抓取策略与非懒抓取的区别仅在于缓存存在但过期的情况。在这种情况下，**非懒抓取策略将优先回源**，保证数据的时效性，回源失败再取缓存；**懒抓取策略将优先取缓存**，保证响应速度，同时在后台回源并更新缓存，用户在一段时间后再次请求，将得到更新后的数据。

这样做的好处是保证每次请求在取出缓存后立即结束，无需等待下游中间件完成任务。
