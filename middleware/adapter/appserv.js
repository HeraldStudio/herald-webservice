      ...
      try {
        ...
        if(/ios/i.test(versiontype)){
          content.version={
            code:64,
            name:' v2.0.0 ',
            des:'\n【兼容性】iOS 10 设备被报告存在少量兼容性问题，如果升级后遇到问题请更新系统。【非常重要】如果提示“项目不可用”，是由于您错过了上一个版本的更新，您需要在 AppStore 手动搜索“小猴偷米”以进行更新\n\n 期待已久的新小猴偷米 iOS App 终于来啦！布局深度调整，纠正一直以来存在的数据不同步问题，更多新功能等你发现～'}
        }
        ...
        content.sliderviews = (await bannerCollection.find({
          startTime: { $lte: now },
          endTime: { $gt: now }
        }).sort('startTime', -1).toArray()).filter(k =>
          schoolnum.indexOf(k.schoolnumPrefix) === 0 ||
          !schoolnum && k.schoolnumPrefix === 'guest' ||
          schoolnum && k.schoolnumPrefix === '!guest'
        ).map(k => {
